...
func (scc *Controller) syncLocalSecrets(
	...
	scyllaDBManagerAgentAuthTokenSources := []controllerhelpers.ScyllaDBManagerAuthTokenSource{
		{
			GetOptionalAuthTokenSecret: getOptionalExistingScyllaDBManagerAuthTokenSecretFunc(sc, localSecrets),
			ExtractAuthTokenFromSecret: helpers.GetAgentAuthTokenFromSecret,
		},
	}

	scyllaDBManagerAgentAuthTokenProgressingConditions, scyllaDBManagerAgentAuthToken, err := controllerhelpers.GetScyllaDBManagerAgentAuthToken(scyllaDBManagerAgentAuthTokenSources)
...
func getOptionalExistingScyllaDBManagerAuthTokenSecretFunc(sc *scyllav1alpha1.ScyllaDBCluster, localSecrets map[string]*corev1.Secret) func() ([]metav1.Condition, *corev1.Secret, error) {
	return func() ([]metav1.Condition, *corev1.Secret, error) {
		...
		if err != nil {
			return progressingConditions, nil, fmt.Errorf("can't get agent auth token secret name for ScyllaDBCluster %q: %w", naming.ObjRef(sc), err)
		}
