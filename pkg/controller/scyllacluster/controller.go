...
func (scc *Controller) enqueueSecretOwnerOrAllUsingItAsCA(depth int, obj kubeinterfaces.ObjectInterface, op controllerhelpers.HandlerOperationType) {
	...
	klog.V(2).InfoS("enqueueSecretOwnerOrAllUsingItAsCA", "Secret", secret)

	sc := scc.resolveScyllaClusterController(secret)
	if sc != nil {
		klog.V(2).InfoS("enqueueSecretOwnerOrAllUsingItAsCA: will enqueue owner", "Secret", secret, "Owner", klog.KObj(sc))
		scc.handlers.Enqueue(depth+1, sc, op)
		return
	}

	klog.V(2).InfoS("enqueueSecretOwnerOrAllUsingItAsCA: will try enqueueing all matching CAs", "Secret", secret)
