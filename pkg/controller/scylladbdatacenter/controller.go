...
func (sdcc *Controller) enqueueSecretOwnerOrAllUsingItAsCA(depth int, obj kubeinterfaces.ObjectInterface, op controllerhelpers.HandlerOperationType) {
	...
	isUserManagedCAOfScyllaDBDatacenter := func(sdc *scyllav1alpha1.ScyllaDBDatacenter) bool {
		...
		if sdc.Spec.CertificateOptions.ClientCA.Type == scyllav1alpha1.TLSCertificateAuthorityTypeUserManaged && sdc.Spec.CertificateOptions.ClientCA.UserManagedOptions.SecretName == secretName {
			return true
		}

		if sdc.Spec.CertificateOptions.ServingCA.Type == scyllav1alpha1.TLSCertificateAuthorityTypeUserManaged && sdc.Spec.CertificateOptions.ServingCA.UserManagedOptions.SecretName == secretName {
			return true
		}
