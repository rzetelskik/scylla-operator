...
func (smcrc *Controller) deleteScyllaDBManagerCluster(ctx context.Context, smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) error {
	...
	managerCluster, found, err := smcrc.getScyllaDBManagerCluster(ctx, smcr, managerClient)
	if err != nil {
		return fmt.Errorf("can't get cluster from ScyllaDB Manager state: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("can't delete cluster from ScyllaDB Manager state: %w", err)
	}
