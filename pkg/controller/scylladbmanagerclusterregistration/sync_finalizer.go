...
func (smcrc *Controller) syncFinalizer(ctx context.Context, smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) ([]metav1.Condition, error) {
	...
	err = smcrc.deleteScyllaDBManagerCluster(ctx, smcr)
	if err != nil {
...
func (smcrc *Controller) deleteScyllaDBManagerCluster(ctx context.Context, smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) error {
	managerClient, err := controllerhelpers.GetScyllaDBManagerClient(ctx, smcr)
	if err != nil {
		return fmt.Errorf("can't get manager client: %w", err)
	}

	managerCluster, found, err := getScyllaDBManagerCluster(ctx, smcr, managerClient)
	if err != nil {
		return fmt.Errorf("can't get ScyllaDB Manager cluster: %w", err)
	}

	if !found {
		klog.V(4).InfoS("ScyllaDB Manager cluster has already been deleted", "ScyllaDBManagerClusterRegistration", klog.KObj(smcr), "UID", smcr.UID)
		return nil
	}

	err = managerClient.DeleteCluster(ctx, managerCluster.ID)
	if err != nil {
		return fmt.Errorf("can't delete ScyllaDB Manager cluster: %w", err)
	}

	klog.V(4).InfoS("Deleted ScyllaDB Manager cluster", "ScyllaDBManagerClusterRegistration", klog.KObj(smcr), "UID", smcr.UID, "ScyllaDBManagerClusterName", managerCluster.Name, "ScyllaDBManagerClusterID", managerCluster.ID)
...
func (smcrc *Controller) removeFinalizer(ctx context.Context, smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) error {
	...
	_, err = smcrc.scyllaClient.ScyllaDBManagerClusterRegistrations(smcr.Namespace).Patch(ctx, smcr.Name, types.MergePatchType, patch, metav1.PatchOptions{})
	if err != nil {
