...
func (smcrc *Controller) sync(ctx context.Context, key string) error {
	...
	err = controllerhelpers.RunSync(
		&status.Conditions,
		registrationControllerProgressingCondition,
		registrationControllerDegradedCondition,
		smcr.Generation,
		func() ([]metav1.Condition, error) {
			return smcrc.syncRegistration(ctx, smcr, status)
		},
	)
	if err != nil {
...
func (smcrc *Controller) getManagerClient(ctx context.Context, smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) (*managerclient.Client, error) {
	if !isManagedByGlobalScyllaDBManagerInstance(smcr) {
		return nil, fmt.Errorf("ScyllaDBManagerClusterRegistration %q is not managed by global ScyllaDB Manager instance", naming.ObjRef(smcr))
	}
...
func isManagedByGlobalScyllaDBManagerInstance(smcr *scyllav1alpha1.ScyllaDBManagerClusterRegistration) bool {
	globalScyllaDBManagerAnnotationValue := smcr.Labels[naming.GlobalScyllaDBManagerLabel]
	return globalScyllaDBManagerAnnotationValue == naming.LabelValueTrue
}
