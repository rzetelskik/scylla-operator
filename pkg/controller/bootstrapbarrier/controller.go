...
func (c *Controller) Sync(ctx context.Context) error {
	...
	bootstrapPreconditionSatisfied := isBootstrapPreconditionSatisfied(scyllaDBDatacenterNodesStatusReports, svcDC, svcRack, svcOrdinal)
...
func isBootstrapPreconditionSatisfied(scyllaDBDatacenterNodesStatusReports []*scyllav1alpha1.ScyllaDBDatacenterNodesStatusReport, selfDC string, selfRack string, selfOrdinal int) bool {
	...
	for _, sdcnsr := range scyllaDBDatacenterNodesStatusReports {
		for _, rack := range sdcnsr.Racks {
			for _, node := range rack.Nodes {
				if sdcnsr.Name == selfDC && rack.Name == selfRack && node.Ordinal == selfOrdinal {
					// Skip self.
				...
				if node.HostID == nil {
					klog.V(4).InfoS("A required node is missing a host ID, can't proceed with bootstrap", "RequiredNodeDatacenter", sdcnsr.Name, "RequiredNodeRack", rack.Name, "RequiredNodeOrdinal", node.Ordinal)
					return false
				}
	...
	if len(scyllaDBDatacenterNodesStatusReports) == 1 && scyllaDBDatacenterNodesStatusReports[0].Name == selfDC {
