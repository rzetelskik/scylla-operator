...
func (cm *CertificateManager) ManageCertificatesWithSigningTLSSecretFunc(
	...
	startTime := time.Now()
	klog.V(4).InfoS("ManageCertificatesWithSigningTLSSecretFunc: starting", "startTime", startTime)
	defer klog.V(4).InfoS("ManageCertificatesWithSigningTLSSecretFunc: finished", "duration", time.Now().Sub(startTime))
...
func (cm *CertificateManager) ManageCertificates(
	ctx context.Context,
	nowFunc func() time.Time,
	controller *metav1.ObjectMeta,
	controllerGVK schema.GroupVersionKind,
