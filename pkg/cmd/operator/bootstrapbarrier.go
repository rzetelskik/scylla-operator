...
func (o *BootstrapBarrierOptions) AddFlags(cmd *cobra.Command) {
	...
	// TODO
	cmd.Flags().StringVarP(&o.ScyllaDBStatusReportSelectorLabelValue, "scylladb-status-report-selector-label-value", "", o.ScyllaDBStatusReportSelectorLabelValue, "")
}
...
func (o *BootstrapBarrierOptions) Validate(args []string) error {
	...
	if len(o.ScyllaDBStatusReportSelectorLabelValue) == 0 {
		errs = append(errs, fmt.Errorf("scylladb-status-report-selector-label-value can't be empty"))
	} else {
		// TODO: verify against actual requirements for a label value
		scyllaDBStatusReportSelectorLabelValueValidationErrs := apimachineryvalidation.NameIsDNSSubdomain(o.ScyllaDBStatusReportSelectorLabelValue, false)
		if len(scyllaDBStatusReportSelectorLabelValueValidationErrs) != 0 {
			errs = append(errs, fmt.Errorf("invalid ScyllaDBStatusReport selector label value %q: %v", o.ScyllaDBStatusReportSelectorLabelValue, scyllaDBStatusReportSelectorLabelValueValidationErrs))
		}
	}
...
func (o *BootstrapBarrierOptions) Execute(cmdCtx context.Context, originalStreams genericclioptions.IOStreams, cmd *cobra.Command) error {
	...
	scyllaDBStatusReportKubeInformers := scyllainformers.NewSharedInformerFactoryWithOptions(
		o.scyllaClient,
		12*time.Hour,
		...
		scyllainformers.WithTweakListOptions(
			func(options *metav1.ListOptions) {
				options.LabelSelector = labels.Set{
					naming.ScyllaDBStatusReportSelectorLabel: o.ScyllaDBStatusReportSelectorLabelValue,
				}.String()
			},
		),
	)
	...
	boostrapBarrierController, err := bootstrapbarrier.NewController(
		o.Namespace,
		o.ServiceName,
		o.ScyllaDBStatusReportSelectorLabelValue,
		boostrapPreconditionCh,
		o.kubeClient,
		identityKubeInformers.Core().V1().Services(),
		scyllaDBStatusReportKubeInformers.Scylla().V1alpha1().ScyllaDBStatusReports(),
	)
	if err != nil {
	...
	go func() {
		defer wg.Done()
		scyllaDBStatusReportKubeInformers.Start(ctx.Done())
	}()
