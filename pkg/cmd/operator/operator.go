...
func (o *OperatorOptions) run(ctx context.Context, streams genericclioptions.IOStreams) error {
	...
	globalScyllaDBManagerDeploymentInformers := informers.NewSharedInformerFactoryWithOptions(o.kubeClient, resyncPeriod, informers.WithNamespace(naming.ScyllaManagerNamespace), informers.WithTweakListOptions(
		func(options *metav1.ListOptions) {
			options.FieldSelector = fields.OneTermEqualSelector("metadata.name", naming.ScyllaManagerDeploymentName).String()
		},
	))
	...
	gsmc, err := globalscylladbmanager.NewController(
		o.kubeClient,
		o.scyllaClient.ScyllaV1alpha1(),
		globalScyllaDBManagerDeploymentInformers.Apps().V1().Deployments(),
	...
	go func() {
		defer wg.Done()
		globalScyllaDBManagerDeploymentInformers.Start(ctx.Done())
	}()
