...
var _ = g.Describe("ScyllaDBCluster integration with global ScyllaDB Manager", framework.MultiDatacenter, func() {
	...
	g.It("should register labeled ScyllaDBCluster and deregister it when it's unlabeled", func(ctx g.SpecContext) {
		...
		ns, nsClient := metaCluster.CreateUserNamespace(ctx)
		...
		sc, err = metaCluster.ScyllaAdminClient().ScyllaV1alpha1().ScyllaDBClusters(ns.Name).Create(ctx, sc, metav1.CreateOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		sc, err = controllerhelpers.WaitForScyllaDBClusterState(rolloutCtx, metaCluster.ScyllaAdminClient().ScyllaV1alpha1().ScyllaDBClusters(sc.Namespace), sc.Name, controllerhelpers.WaitForStateOptions{}, utils.IsScyllaDBClusterRolledOut)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		managerClient, err := utils.GetManagerClient(ctx, metaCluster.KubeAdminClient().CoreV1())
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		sc, err = metaCluster.ScyllaAdminClient().ScyllaV1alpha1().ScyllaDBClusters(ns.Name).Patch(
			ctx,
			sc.Name,
	...
	g.It("should register labeled ScyllaDBCluster and deregister it when it's deleted", func(ctx g.SpecContext) {
		rkcs, rkcClusterMap, err := utils.SetUpRemoteKubernetesClustersFromRestConfigs(ctx, framework.TestContext.RestConfigs, f)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		err = metaCluster.ScyllaAdminClient().ScyllaV1alpha1().ScyllaDBClusters(ns.Name).Delete(
			ctx,
			sc.Name,
