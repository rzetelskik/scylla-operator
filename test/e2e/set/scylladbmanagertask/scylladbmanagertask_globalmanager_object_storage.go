...
var _ = g.Describe("ScyllaDBManagerTask integration with global ScyllaDB Manager", func() {
	...
	g.DescribeTable("should synchronise a backup task for ScyllaDBDatacenter and support a manual restore procedure", func(ctx g.SpecContext, e entry) {
		...
		framework.By("Waiting for ScyllaDBManagerTask update to propagate to ScyllaDB Manager state")
		o.Eventually(func(eo o.Gomega, ctx context.Context) {
			managerTask, err := managerClient.GetTask(ctx, sourceManagerClusterID, managerclient.BackupTask, managerTaskID)
			eo.Expect(err).NotTo(o.HaveOccurred())
			eo.Expect(managerTask.Properties.(map[string]interface{})["retention"].(json.Number).Int64()).To(o.Equal(*smt.Spec.Backup.Retention))
		}).WithContext(ctx).WithTimeout(utils.SyncTimeout).WithPolling(3 * time.Second).Should(o.Succeed())

		var backupProgress managerclient.BackupProgress
		framework.By("Waiting for the backup task to finish")
		err = wait.PollUntilContextTimeout(ctx, 5*time.Second, 3*time.Minute, true, func(context.Context) (done bool, err error) {
		...
		framework.By("Waiting for the task to be removed from the global ScyllaDB Manager state")
		o.Eventually(func(eo o.Gomega, ctx context.Context) {
